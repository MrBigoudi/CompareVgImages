...
let q2_5 () =
  ...
  let module Code = struct 
    let gen_matrix = Get.value "gen_matrix" [%ty : (int * int * int * int) list -> int array array]
    let draw_tetris = Get.value "draw_tetris" [%ty : int array array -> Vg.image]
  end in
  Check.expr1 
      [%code (fun list -> Code.draw_tetris@@Code.gen_matrix@@list)]
...
let q3 () =
  ...
  Assume.compatible "gen_matrix_gravity" [%ty : (int * int * int) list -> int array array];
  Check.name1 "gen_matrix_gravity" [%ty : (int * int * int) list -> int array array]
  ...
  (* Test affichage *)
  let module Code = struct 
    let gen_matrix_gravity = Get.value "gen_matrix_gravity" [%ty : (int * int * int) list -> int array array]
    let draw_tetris = Get.value "draw_tetris" [%ty : int array array -> Vg.image]
  end in
  Check.expr1 
      [%code fun list -> Code.draw_tetris@@Code.gen_matrix_gravity@@list ]
